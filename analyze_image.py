...
def openfile_and_process(raw_canvas, cooked_canvas, textbox):
    # openfile
    img_path = filedialog.askopenfilename(filetypes=[('png', '*.png'),('jpg', '*.jpg'),('gif', '*.gif')])  # 指定開啟檔案格式, 回傳一個檔案路徑
    if img_path.split('.')[-1] not in ['jps', 'png', '.gif'] and img_path:
        textbox.delete(1.0,'end')
        textbox.insert(tk.END, "Wrong file type.")
    elif img_path != "":
...
def download_processed_image(cooked_canvas):
...
if __name__ == '__main__':
    ...
    button = tk.Button(
        ...
        font=('Arial',15,'bold'),
        command=lambda :download_processed_image(cooked_canvas)
    )
    button.pack(padx=10, pady=5, fill='x')
